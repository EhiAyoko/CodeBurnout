...
createAuthorizationHeader(headers: HttpHeaders) {
  ...
  this.afStore.collection('/Records/').snapshotChanges().subscribe(res=>{
  console.log('res',res)
...
firebaseEvent(val,value, postimg, name){
  this.brandsdetails
  console.log("vslueOfToggle",val.detail.checked)
  console.log("vslueOfObj",value);
  if(val.detail.checked === true){
    ...
    if(val.detail.checked === false){
      this.afStore.doc('/Team_activities_saved/' + name).delete()
    }
  }
...
DeleteRecord(id){
  this.afStore.doc('/Records/'+id).delete()
}
