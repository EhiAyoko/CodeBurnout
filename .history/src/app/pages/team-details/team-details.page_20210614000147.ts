...
// working for save activity
  ...
  firebaseEvent(val, value, postimg) {
    ...
    if (val.detail.checked === true) {
      ...
      if (this.brandsdetails && this.brandsdetails?.fields?.Video?.url != undefined) {
        addrecord['Video'] = this.brandsdetails?.fields?.Video?.url
    console.log('video', this.brandsdetails?.fields?.Video?.url)
