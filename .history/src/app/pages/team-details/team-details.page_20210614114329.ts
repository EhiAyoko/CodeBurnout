...
// working for save activity
  ...
  firebaseEvent(val, value, postimg) {
    ...
    if (val.detail.checked === true) {
      ...
      if (this.brandsdetails &&  this.brandsdetails?.fields?.Video[0].urll != undefined && 0 == undefined) {
        addrecord['Video'] = this.brandsdetails?.fields?.Video[0].urll
    console.log('video', this.brandsdetails?.fields?.Video[0].urll)
