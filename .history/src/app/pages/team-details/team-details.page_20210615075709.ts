...
export class TeamDetailsPage implements OnInit {
  ...
  async sShare(postimg, brandsdetails){
    ...
    var options = {
      message: brandsdetails.fields.Name, // not supported on some apps (Facebook, Instagram)
      text: brandsdetails.fields.description, // fi. for email
      url: postimg.url,
      chooserTitle: 'aaa', // Android only, you can override the default share sheet title
...
// working for save activity
  ...
  firebaseEvent(val, value, postimg) {
    ...
    if (val.detail.checked === true) {
      ...
      if (this.brandsdetails &&  this.brandsdetails?.fields?.Video[0]?.url != undefined) {
