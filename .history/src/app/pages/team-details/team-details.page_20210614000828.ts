...
// working for save activity
  ...
  onClick(postvideo){
    this.vidoeget = postvideo 
    console.log('vidoeget', this.vidoeget.video.url)
  }

  firebaseEvent(val, value, postimg) {
    this.onClick(this.brandsdetails)
    this.brandsdetails
    console.log("vslueOfToggle", val.detail.checked)
    console.log("vslueOfObj", value);
    if (val.detail.checked === true) {
      ...
      if (this.brandsdetails && this.vidoeget?.url != undefined) {
        this.onClick(this.brandsdetails)
        addrecord['Video'] = this.vidoeget?.url
    console.log('video', this.vidoeget?.url)
