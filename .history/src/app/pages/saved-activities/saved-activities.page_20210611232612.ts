...
this.afStore
      .collection('Explore_activities_saved', (ref) => ref.where('uid', '==',  this.userid))
      .get()
      .subscribe((ss) => {
        let message;
        let single;
        if (ss.docs.length === 0) {
          ...
          ss.docs.forEach((doc) => {
            ...
            this.explorerecords.push(this.teamobject);
          });
...
this.afStore
      .collection('Team_activities_saved', (ref) => ref.where('uid', '==',  this.userid))
      .get()
      .subscribe((ss) => {
        let message;
        let single;
