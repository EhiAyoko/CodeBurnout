...
export class SavedActivitiesPage implements OnInit {
  ...
  teamrecords: { id: string; Name: any; TypeOfActivity: any; url: any; }[];
  currentUser: any;
  userid: any;
...
this.afStore.firestore.collection('Team_activities_saved').doc(this.userid)
.get()
.then(doc => {
  if (doc.exists) {
    this.team = this.teamrecords 
